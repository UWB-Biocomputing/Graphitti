...
/// @param[out] allVerticesDevice          Memory location of the pointer to the
/// neurons list on device memory.
/// @param[out] allEdgesDevice         Memory location of the pointer to the
/// synapses list on device memory.
void GPUModel::allocDeviceStruct(void **allVerticesDevice,
  ...
  BGSIZE randNoise_d_size =
      numVertices * sizeof(float); // size of random noise array
...
void GPUModel::setupSim() {
  ...
  // assuming numVertices >= 100 and is a multiple of 100. Note rng_mt_rng_count
  // must be <= MT_RNG_COUNT
  int rng_blocks = 25; //# of blocks the kernel will use
  int rng_nPerRng = 4; //# of iterations per thread (thread granularity, # of
                       // rands generated per thread)
  ...
  initMTGPU(Simulator::getInstance().getNoiseRngSeed(), rng_blocks, rng_threads,
            rng_nPerRng, rng_mt_rng_count);
  ...
  layout_->getVertices()->setAdvanceVerticesDeviceParams(
...
void GPUModel::advance() {
  ...
  dynamic_cast<AllSpikingNeurons *>(neurons.get())
      ->advanceVertices(*(connections_->getEdges().get()), allVerticesDevice_,
                        allEdgesDevice_, randNoise_d, synapseIndexMapDevice_);
  ...
  // Advance synapses ------------->
  synapses->advanceEdges(allEdgesDevice_, allVerticesDevice_,
...
void GPUModel::calcSummationMap() {
  ...
  int blocksPerGrid =
      (Simulator::getInstance().getTotalVertices() + threadsPerBlock - 1) /
      threadsPerBlock;

  calcSummationMapDevice<<<blocksPerGrid, threadsPerBlock>>>(
      Simulator::getInstance().getTotalVertices(), allVerticesDevice_,
      synapseIndexMapDevice_, allEdgesDevice_);
}
...
void GPUModel::updateConnections() {
  ...
  if (connections_->updateConnections(*(neurons.get()), layout_.get())) {
    connections_->updateSynapsesWeights(
        Simulator::getInstance().getTotalVertices(), *(neurons.get()),
        *(synapses.get()), allVerticesDevice_, allEdgesDevice_, layout_.get());
...
void GPUModel::updateHistory() {
  ...
  shared_ptr<AllVertices> neurons = layout_->getVertices();
  dynamic_cast<AllSpikingNeurons *>(neurons.get())
      ->clearNeuronSpikeCounts(allVerticesDevice_);
}
...
void GPUModel::allocSynapseImap(int count) {
  ...
  HANDLE_ERROR(cudaMalloc((void **)&synapseIMapDevice.outgoingEdgeBegin_,
                          count * sizeof(BGSIZE)));
  HANDLE_ERROR(cudaMalloc((void **)&synapseIMapDevice.outgoingEdgeCount_,
                          count * sizeof(BGSIZE)));
  HANDLE_ERROR(cudaMemset(synapseIMapDevice.outgoingEdgeBegin_, 0,
                          count * sizeof(BGSIZE)));
  HANDLE_ERROR(cudaMemset(synapseIMapDevice.outgoingEdgeCount_, 0,
                          count * sizeof(BGSIZE)));

  HANDLE_ERROR(cudaMalloc((void **)&synapseIMapDevice.incomingEdgeBegin_,
                          count * sizeof(BGSIZE)));
  HANDLE_ERROR(cudaMalloc((void **)&synapseIMapDevice.incomingEdgeCount_,
                          count * sizeof(BGSIZE)));
  HANDLE_ERROR(cudaMemset(synapseIMapDevice.incomingEdgeBegin_, 0,
                          count * sizeof(BGSIZE)));
  HANDLE_ERROR(cudaMemset(synapseIMapDevice.incomingEdgeCount_, 0,
                          count * sizeof(BGSIZE)));

  HANDLE_ERROR(
      cudaMalloc((void **)&synapseIndexMapDevice_, sizeof(EdgeIndexMap)));
...
/// @param  synapseIndexMapHost		Reference to the EdgeIndexMap in host
/// memory.
void GPUModel::copySynapseIndexMapHostToDevice(
    EdgeIndexMap &synapseIndexMapHost, int numVertices) {
  shared_ptr<AllEdges> synapses = connections_->getEdges();
  int totalSynapseCount =
      dynamic_cast<AllEdges *>(synapses.get())->totalEdgeCount_;
  ...
  HANDLE_ERROR(cudaMemcpy(synapseIMapDevice.incomingEdgeBegin_,
                          synapseIndexMapHost.incomingEdgeBegin_,
                          numVertices * sizeof(BGSIZE),
                          cudaMemcpyHostToDevice));
  HANDLE_ERROR(cudaMemcpy(synapseIMapDevice.incomingEdgeCount_,
                          synapseIndexMapHost.incomingEdgeCount_,
                          numVertices * sizeof(BGSIZE),
                          cudaMemcpyHostToDevice));
  // the number of synapses may change, so we reallocate the memory
...
/// contiguously.
///
/// @param[in] totalVertices           Number of vertices in the entire
/// simulation.
/// @param[in,out] allVerticesDevice   Pointer to Neuron structures in device
/// memory.
/// @param[in] synapseIndexMapDevice_  Pointer to forward map structures in
/// device memory.
/// @param[in] allEdgesDevice      Pointer to Synapse structures in device
/// memory.
__global__ void calcSummationMapDevice(
    int totalVertices,
