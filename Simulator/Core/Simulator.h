...
public:
  ...
  void advanceEpoch(
      const int &currentEpoch) const; /// Advance simulation to next growth
                                      /// cycle. Helper for #simulate().

  void
  saveResults() const; /// Writes simulation results to an output destination.
  ...
  int getMaxEdgesPerVertex()
      const; /// Maximum number of synapses per neuron. **GPU Only**
  ...
  long getNoiseRngSeed()
      const; /// Seed used for the simulation random **SingleThreaded Only**

  long getInitRngSeed() const; /// Seed used to initialize parameters

  string
  getConfigFileName() const; /// File name of the parameter configuration file.

  string
  getSerializationFileName() const; /// File name of the serialization file.

  string
  getDeserializationFileName() const; /// File name of the deserialization file.
  ...
  Timer &
  getShort_timer(); /// Timer for measuring performance of connection update.
#endif
...
private:
  ...
  shared_ptr<Model>
      model_; /// Smart pointer to model class (Model is an interface class)

  log4cplus::Logger consoleLogger_; /// Logger for printing to the console as
                                    /// well as the logging file
