...
Simulator::Simulator() {
  ...
  // OperationManager
  function<void()> printParametersFunc =
      bind(&Simulator::printParameters, this);
...
void Simulator::loadParameters() {
  ...
  ParameterManager::getInstance().getBGFloatByXpath(
      "//SimParams/epochDuration/text()", epochDuration_);
  ParameterManager::getInstance().getIntByXpath("//SimParams/numEpochs/text()",
                                                numEpochs_);
  ParameterManager::getInstance().getIntByXpath(
      "//SimConfig/maxFiringRate/text()", maxFiringRate_);
  ParameterManager::getInstance().getIntByXpath(
      "//SimConfig/maxEdgesPerVertex/text()", maxEdgesPerVertex_);

  // Instantiate rng object
  string type;
  ParameterManager::getInstance().getStringByXpath(
      "//RNGConfig/NoiseRNGSeed/@class", type);
  noiseRNG = RNGFactory::getInstance().createRNG(type);

  ParameterManager::getInstance().getLongByXpath(
      "//RNGConfig/InitRNGSeed/text()", initRngSeed_);
  ParameterManager::getInstance().getLongByXpath(
      "//RNGConfig/NoiseRNGSeed/text()", noiseRngSeed_);
...
void Simulator::printParameters() const {
  LOG4CPLUS_DEBUG(fileLogger_,
                  "\nSIMULATION PARAMETERS"
                      ...
                      << "\tTime between growth updates (in seconds): "
                      << epochDuration_ << endl
                      << "\tNumber of epochs to run: " << numEpochs_ << endl
                      << "\tMax firing rate: " << maxFiringRate_ << endl
                      << "\tMax edges per vertex: " << maxEdgesPerVertex_
                      << endl
...
void Simulator::simulate() {
  ...
  for (int currentEpoch = 1; currentEpoch <= numEpochs_; currentEpoch++) {
    LOG4CPLUS_TRACE(consoleLogger_,
                    "Performing epoch number: " << currentEpoch);
...
#endif
    LOG4CPLUS_TRACE(consoleLogger_, "done with epoch cycle "
                                        << currentEpoch_
                                        << ", beginning growth update");
...
/// Helper for #simulate(). Advance simulation until ready for next growth
/// cycle. This should simulate all neuron and synapse activity for one epoch.
/// @param currentStep the current epoch in which the network is being
/// simulated.
void Simulator::advanceEpoch(const int &currentEpoch) const {
  uint64_t count = 0;
  // Compute step number at end of this simulation epoch
  uint64_t endStep =
      g_simulationStep + static_cast<uint64_t>(epochDuration_ / deltaT_);
...
bool Simulator::instantiateSimulatorObjects() {
  ...
  if (!model_ || !model_->getConnections() ||
      !model_->getConnections()->getEdges() || !model_->getLayout() ||
      !model_->getLayout()->getVertices() || !model_->getRecorder()) {
    return false;
  }
  return true;
}
