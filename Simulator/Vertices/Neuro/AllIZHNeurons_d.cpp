...
///  @param[in] allVerticesDevice      Pointer to Neuron structures in device
///  memory.
///  @param[in] allEdgesDevice     Pointer to Synapse structures in device
///  memory.
///  @param[in] edgeIndexMap       Inverse map, which is a table indexed by an
///  input neuron and maps to the synapses that provide input to that neuron.
///  @param[in] fAllowBackPropagation True if back propagaion is allowed.

__global__ void advanceIZHNeuronsDevice(
    int totalVertices, int maxEdges, int maxSpikes, const BGFLOAT deltaT,
    uint64_t simulationStep, float *randNoise,
    AllIZHNeuronsDeviceProperties *allVerticesDevice,
    AllSpikingSynapsesDeviceProperties *allEdgesDevice,
    EdgeIndexMap *edgeIndexMapDevice, bool fAllowBackPropagation);

///  Allocate GPU memories to store all neurons' states,
///  and copy them from host to GPU memory.
///
///  @param  allVerticesDevice   GPU address of the
///  AllIZHNeuronsDeviceProperties struct
///                             on device memory.
void AllIZHNeurons::allocNeuronDeviceStruct(void **allVerticesDevice) {
  ...
  HANDLE_ERROR(
      cudaMalloc(allVerticesDevice, sizeof(AllIZHNeuronsDeviceProperties)));
  HANDLE_ERROR(cudaMemcpy(*allVerticesDevice, &allVerticesDeviceProps,
                          sizeof(AllIZHNeuronsDeviceProperties),
...
///
///  @param  allVerticesDevice    GPU address of the
///  AllIZHNeuronsDeviceProperties struct on device memory.
void AllIZHNeurons::allocDeviceStruct(
    AllIZHNeuronsDeviceProperties &allVerticesDevice) {
  ...
  HANDLE_ERROR(
      cudaMalloc((void **)&allVerticesDevice.Aconst_, count * sizeof(BGFLOAT)));
  HANDLE_ERROR(
      cudaMalloc((void **)&allVerticesDevice.Bconst_, count * sizeof(BGFLOAT)));
  HANDLE_ERROR(
      cudaMalloc((void **)&allVerticesDevice.Cconst_, count * sizeof(BGFLOAT)));
  HANDLE_ERROR(
      cudaMalloc((void **)&allVerticesDevice.Dconst_, count * sizeof(BGFLOAT)));
  HANDLE_ERROR(
      cudaMalloc((void **)&allVerticesDevice.u_, count * sizeof(BGFLOAT)));
  HANDLE_ERROR(
      cudaMalloc((void **)&allVerticesDevice.C3_, count * sizeof(BGFLOAT)));
}
...
///  @param  allVerticesDevice    GPU address of the
///  AllIZHNeuronsDeviceProperties struct on device memory.
void AllIZHNeurons::deleteDeviceStruct(
    AllIZHNeuronsDeviceProperties &allVerticesDevice) {
...
///  @param  allVerticesDevice    GPU address of the
///  AllIZHNeuronsDeviceProperties struct on device memory.
void AllIZHNeurons::copyHostToDevice(
    AllIZHNeuronsDeviceProperties &allVerticesDevice) {
  int count = Simulator::getInstance().getTotalVertices();

  AllIFNeurons::copyHostToDevice(allVerticesDevice);

  HANDLE_ERROR(cudaMemcpy(allVerticesDevice.Aconst_, Aconst_,
                          count * sizeof(BGFLOAT), cudaMemcpyHostToDevice));
  HANDLE_ERROR(cudaMemcpy(allVerticesDevice.Bconst_, Bconst_,
                          count * sizeof(BGFLOAT), cudaMemcpyHostToDevice));
  HANDLE_ERROR(cudaMemcpy(allVerticesDevice.Cconst_, Cconst_,
                          count * sizeof(BGFLOAT), cudaMemcpyHostToDevice));
  HANDLE_ERROR(cudaMemcpy(allVerticesDevice.Dconst_, Dconst_,
                          count * sizeof(BGFLOAT), cudaMemcpyHostToDevice));
  HANDLE_ERROR(cudaMemcpy(allVerticesDevice.u_, u_, count * sizeof(BGFLOAT),
                          cudaMemcpyHostToDevice));
  HANDLE_ERROR(cudaMemcpy(allVerticesDevice.C3_, C3_, count * sizeof(BGFLOAT),
                          cudaMemcpyHostToDevice));
}
...
///  @param  allVerticesDevice    GPU address of the
///  AllIZHNeuronsDeviceProperties struct on device memory.
void AllIZHNeurons::copyDeviceToHost(
    AllIZHNeuronsDeviceProperties &allVerticesDevice) {
  int count = Simulator::getInstance().getTotalVertices();

  AllIFNeurons::copyDeviceToHost(allVerticesDevice);

  HANDLE_ERROR(cudaMemcpy(Aconst_, allVerticesDevice.Aconst_,
                          count * sizeof(BGFLOAT), cudaMemcpyDeviceToHost));
  HANDLE_ERROR(cudaMemcpy(Bconst_, allVerticesDevice.Bconst_,
                          count * sizeof(BGFLOAT), cudaMemcpyDeviceToHost));
  HANDLE_ERROR(cudaMemcpy(Cconst_, allVerticesDevice.Cconst_,
                          count * sizeof(BGFLOAT), cudaMemcpyDeviceToHost));
  HANDLE_ERROR(cudaMemcpy(Dconst_, allVerticesDevice.Dconst_,
                          count * sizeof(BGFLOAT), cudaMemcpyDeviceToHost));
  HANDLE_ERROR(cudaMemcpy(u_, allVerticesDevice.u_, count * sizeof(BGFLOAT),
                          cudaMemcpyDeviceToHost));
  HANDLE_ERROR(cudaMemcpy(C3_, allVerticesDevice.C3_, count * sizeof(BGFLOAT),
                          cudaMemcpyDeviceToHost));
}
...
///  AllIZHNeuronsDeviceProperties struct
///                             on device memory.
void AllIZHNeurons::copyNeuronDeviceSpikeHistoryToHost(
...
__global__ void advanceIZHNeuronsDevice(
  ...
  if (allVerticesDevice->numStepsInRefractoryPeriod_[idx] >
      0) { // is neuron refractory?
    ...
    if (synapseCounts != 0) {
      ...
      BGSIZE *outgoingMapBegin =
          &(edgeIndexMapDevice->outgoingEdgeIndexMap_[beginIndex]);
    ...
    if (fAllowBackPropagation && synapseCounts != 0) {
      ...
      // get the memory location of where that list begins
      BGSIZE *incomingMapBegin =
          &(edgeIndexMapDevice->incomingEdgeIndexMap_[beginIndex]);
      ...
      case classAllDSSynapses:
        for (BGSIZE i = 0; i < synapseCounts; i++) {
          postSpikingSynapsesSpikeHitDevice(incomingMapBegin[i],
                                            allEdgesDevice);
        } // end for
        break;
    ...
    r_sp +=
        (randNoise[idx] * allVerticesDevice->Inoise_[idx]); // add cheap noise

    BGFLOAT Vint = r_vm * 1000;

    // Izhikevich model integration step
    BGFLOAT Vb = Vint + allVerticesDevice->C3_[idx] *
                            (0.04 * Vint * Vint + 5 * Vint + 140 - u);
