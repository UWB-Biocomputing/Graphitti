 ...
 *
 * The class uses a data-centric structure, which utilizes a structure as the
 *containers of all neuron.
 *
 ...
 * simulator, and this refactored version of the simulator simply uses that
 *design for all other implementations as well. This is to simplify
 *transitioning from single-threaded to multi-threaded.
 *
 ...
 * \f[
 *   \tau_m \frac{d V_m}{dt} = -(V_m-V_{resting}) + R_m \cdot
 *(I_{syn}(t)+I_{inject}+I_{noise}) \f] where \f$\tau_m=C_m\cdot R_m\f$ is the
 *membrane time constant, \f$R_m\f$ is the membrane resistance, \f$I_{syn}(t)\f$
 *is the current supplied by the synapses, \f$I_{inject}\f$ is a non-specific
 *background current and \f$I_{noise}\f$ is a Gaussian random variable with zero
 *mean and a given variance noise.
 *
 ...
 * \f[
 *  y(t+\Delta t) = y(t) \cdot \mathrm{e}^{-B \Delta t} + \frac{A}{B} \cdot (1 -
 *\mathrm{e}^{-B \Delta t}) \f]
 *
 ...
 * \f]
 * \f[
 *  A = \frac{1}{\tau_m} \cdot (R_m \cdot (I_{syn}(t)+I_{inject}+I_{noise}) +
 *V_{resting}) \f] and \f[ B = \frac{1}{\tau_m} \f]
 *
...
public:
  ...
  ///  memory.
  virtual void advanceVertices(AllEdges &synapses, void *allVerticesDevice,
                               void *allEdgesDevice, float *randNoise,
                               EdgeIndexMap *edgeIndexMapDevice) override;
