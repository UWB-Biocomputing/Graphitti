...
void AllSpikingNeurons::copyToDevice(void *deviceAddress)
{
   ...
   HANDLE_ERROR(cudaMemcpy(&allVerticesDevice, deviceAddress,sizeof(AllSpikingNeuronsDeviceProperties), cudaMemcpyDeviceToHost));
   ...
   HANDLE_ERROR(cudaMemcpy(allVerticesDevice.hasFired_, cpu_has_fired, count * sizeof(bool), cudaMemcpyHostToDevice));

   int cpu_spike_count[count];
   for (int i = 0; i < count; i++) {
      cpu_spike_count[i] = vertexEvents_[i].getNumEventsInEpoch();
   }
   HANDLE_ERROR(cudaMemcpy(allVerticesDevice.numEventsInEpoch_, cpu_spike_count, count * sizeof(int), cudaMemcpyHostToDevice));

   int cpu_queue_front[count];
   for (int i = 0; i < count; i++) {
      cpu_queue_front[i] = vertexEvents_[i].queueFront_;
   }
   HANDLE_ERROR(cudaMemcpy(allVerticesDevice.queueFront_, cpu_queue_front, count * sizeof(int), cudaMemcpyHostToDevice));

   int cpu_queue_end[count];
   for (int i = 0; i < count; i++) {
      cpu_queue_end[i] = vertexEvents_[i].queueEnd_;
   }
   HANDLE_ERROR(cudaMemcpy(allVerticesDevice.queueEnd_, cpu_queue_end, count * sizeof(int), cudaMemcpyHostToDevice));
   int cpu_queue_start[count];
   for (int i = 0; i < count; i++) {
      cpu_queue_start[i] = vertexEvents_[i].epochStart_;
   }
   HANDLE_ERROR(cudaMemcpy(allVerticesDevice.epochStart_, cpu_queue_start, count * sizeof(int), cudaMemcpyHostToDevice));
   ...
   for (int i = 0; i < count; i++) {
      HANDLE_ERROR(cudaMemcpy(pSpikeHistory[i], vertexEvents_[i].eventTimeSteps_.data(), maxSpikes * sizeof(uint64_t), cudaMemcpyHostToDevice));
   } 
}
void AllSpikingNeurons::copyFromDevice(void *deviceAddress)
{
   ...
   HANDLE_ERROR(cudaMemcpy(cpu_has_fired,  allVerticesDevice.hasFired_, numVertices * sizeof(bool), cudaMemcpyDeviceToHost));
   ...
   HANDLE_ERROR(cudaMemcpy(cpu_spike_count, allVerticesDevice.numEventsInEpoch_, numVertices * sizeof(int), cudaMemcpyDeviceToHost));
   for (int i = 0; i < numVertices; i++) {
   ...
   int maxSpikes = vertexEvents_[0].eventTimeSteps_.size() - 1;
   for (int i = 0; i < numVertices; i++) {
      HANDLE_ERROR(cudaMemcpy(vertexEvents_[i].eventTimeSteps_.data(), pSpikeHistory[i], maxSpikes * sizeof(uint64_t*), cudaMemcpyDeviceToHost));
   }
}
