...
void AllSpikingNeurons::copyToDevice( void * deviceAddress)
{
   ...
   for (int i = 0; i < count; i++) {
      HANDLE_ERROR(cudaMemcpy(pSpikeHistory[i], vertexEvents_[i].eventTimeSteps_.data() , maxSpikes * sizeof(u_int64_t),
                              cudaMemcpyHostToDevice));
   } 
}
...
void AllSpikingNeurons::copyFromDevice(void * deviceAddress)
{
   ...
   int maxSpikes = vertexEvents_[0].eventTimeSteps_.size() - 1;
   for (int i = 0; i < numVertices; i++) {
      HANDLE_ERROR(cudaMemcpy(vertexEvents_[i].eventTimeSteps_.data(), pSpikeHistory[i], maxSpikes * sizeof(u_int64_t),
                              cudaMemcpyDeviceToHost));
   }
}
