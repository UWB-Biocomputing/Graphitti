...
void SparseMatrix::HashTable::resize(int s, int c)
{
   ...
   if ((s == capacity) && (c == columns)) {
      DEBUG_SPARSE(cerr << "\t\t\tSM::HT::resize(): table capacity unchanged; filling with nullptr."
                        << endl;)
...
void SparseMatrix::HashTable::insert(Element *el)
{
   ...
   DEBUG_SPARSE(cerr << "\tInserting value " << el->value << " at (" << el->row << ", "
                     << el->column << ")" << endl;)
   ...
   theElements(MaxElements(r, c), c, this)
{
   ...
   for (TiXmlElement *rowElement = e->FirstChildElement("Row"); rowElement != nullptr;
...
SparseMatrix::SparseMatrix(const SparseMatrix &oldM) :
   Matrix("sparse", "none", oldM.rows, oldM.columns, oldM.multiplier), theRows(nullptr),
   theColumns(nullptr), theElements(MaxElements(oldM.rows, oldM.columns), oldM.columns, this)
{
...
SparseMatrix &SparseMatrix::operator=(const SparseMatrix &rhs)
{
   ...
   SetAttributes(rhs.type, rhs.init, rhs.rows, rhs.columns, rhs.multiplier, rhs.dimensions);
...
void SparseMatrix::clear(void)
{
   ...
   if (theRows != nullptr) {
      for (int i = 0; i < rows; i++)
         for (list<Element *>::iterator it = theRows[i].begin(); it != theRows[i].end(); it++)
            delete *it;
      delete[] theRows;
   }
...
void SparseMatrix::copy(const SparseMatrix &source)
{
   ...
   for (int i = 0; i < rows; i++) {
      for (list<Element *>::iterator it = source.theRows[i].begin(); it != source.theRows[i].end();
           it++) {
...
void SparseMatrix::rowFromXML(TiXmlElement *rowElement)
{
   ...
   // and hash table
   for (TiXmlElement *child = rowElement->FirstChildElement("Entry"); child != nullptr;
...
void SparseMatrix::alloc(void)
{
   ...
   DEBUG_SPARSE(cerr << "\t\tStorage allocated for " << rows << " row by " << columns
                     << " column SparseMatrix." << endl;)
}
...
void SparseMatrix::Print(ostream &os) const
{
   ...
   for (int i = 0; i < rows; i++) {
      ...
      for (list<Element *>::iterator it = theRows[i].begin(); it != theRows[i].end(); it++) {
         ...
         if ((*it)->value != 0.0)
            os << "      <Entry number=\"" << (*it)->column << "\" value=\"" << (*it)->value
               << "\"/>" << endl;
      }
      os << "   </Row>" << endl;
   }
...
string SparseMatrix::toXML(string name) const
{
   ...
   if (name != "")
      os << "name=\"" << name << "\" ";
   os << "type=\"sparse\" rows=\"" << rows << "\" columns=\"" << columns << "\" ";
...
const SparseMatrix SparseMatrix::operator-() const
{
   ...
   for (int i = 0; i < result.rows; i++)
      for (list<Element *>::iterator it = result.theRows[i].begin(); it != result.theRows[i].end();
           it++)
         (*it)->value = -(*it)->value;
