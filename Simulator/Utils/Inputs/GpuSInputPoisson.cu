...
GpuSInputPoisson::GpuSInputPoisson(SimulationInfo *psi, TiXmlElement *parms)
    : SInputPoisson(psi, parms) {}
...
void GpuSInputPoisson::init(SimulationInfo *psi) {
  ...
  // setup seeds
  setupSeeds<<<blocksPerGrid, threadsPerBlock>>>(vertex_count, devStates_d,
                                                 time(nullptr));
}
...
void GpuSInputPoisson::inputStimulus(SimulationInfo *psi) {
  ...
  inputStimulusDevice<<<blocksPerGrid, threadsPerBlock>>>(
      vertex_count, nISIs_d, masks_d, psi->deltaT, lambda, devStates_d,
      allEdgesDevice);

  // advance synapses
  advanceSpikingSynapsesDevice<<<blocksPerGrid, threadsPerBlock>>>(
      edge_count, edgeIndexMapDevice, g_simulationStep, psi->deltaT,
      (AllSpikingSynapsesDeviceProperties *)allEdgesDevice);

  // update summation point
  applyI2SummationMap<<<blocksPerGrid, threadsPerBlock>>>(
      vertex_count, psi->pSummationMap, allEdgesDevice);
}
...
void GpuSInputPoisson::allocDeviceValues(IModel *model, SimulationInfo *psi,
                                         int *nISIs) {
  int vertex_count = psi->totalVertices;
  ...
  HANDLE_ERROR(
      cudaMemcpy(nISIs_d, nISIs, nISIs_d_size, cudaMemcpyHostToDevice));
  ...
  initSynapsesDevice<<<blocksPerGrid, threadsPerBlock>>>(
      vertex_count, allEdgesDevice, psi->pSummationMap, psi->width, psi->deltaT,
      weight);
  ...
  HANDLE_ERROR(cudaMalloc((void **)&edgeIndexMap.incomingSynapseIndexMap,
                          vertex_count * sizeof(BGSIZE)));
  HANDLE_ERROR(
      cudaMemcpy(edgeIndexMap.incomingSynapseIndexMap, incomingSynapseIndexMap,
                 vertex_count * sizeof(BGSIZE), cudaMemcpyHostToDevice));
  HANDLE_ERROR(cudaMalloc((void **)&edgeIndexMapDevice, sizeof(EdgeIndexMap)));
  HANDLE_ERROR(cudaMemcpy(edgeIndexMapDevice, &edgeIndexMap,
  ...
  HANDLE_ERROR(cudaMemcpy(masks_d, masks, vertex_count * sizeof(bool),
...
void GpuSInputPoisson::deleteDeviceValues(IModel *model, SimulationInfo *psi) {
  ...
  EdgeIndexMap edgeIndexMap;
  HANDLE_ERROR(cudaMemcpy(&edgeIndexMap, edgeIndexMapDevice,
...
__global__ void
inputStimulusDevice(int n, int *nISIs_d, bool *masks_d, BGFLOAT deltaT,
                    BGFLOAT lambda, curandState *devStates_d,
...
/// @param[in] allEdgesDevice  Pointer to Synapse structures in device memory.
__global__ void
applyI2SummationMap(int n, BGFLOAT *summationPoint_d,
                    AllDSSynapsesDeviceProperties *allEdgesDevice) {
  int idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (idx >= n)
    return;
...
/// @param[in] devStates_d        Curand global state
/// @param[in] seed               Seed
__global__ void setupSeeds(int n, curandState *devStates_d,
                           unsigned long seed) {
  int idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (idx >= n)
    return;
