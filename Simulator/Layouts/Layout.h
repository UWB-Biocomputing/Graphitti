...
public:
  ...
  /// OperationManager as Operation::loadParameters
  virtual void loadParameters();

  /// Prints out all parameters to logging file. Registered to OperationManager
  /// as Operation::printParameters
  ...
  /// @param    srcVertex  integer that points to a Neuron in the type map as a
  /// source.
  /// @param    destVertex integer that points to a Neuron in the type map as a
  /// destination.
  ...
  vector<int>
      probedNeuronList_; ///< Probed neurons list. // ToDo: Move this to Hdf5
                         ///< recorder once its implemented in project -chris
  ...
  BGSIZE
      numEndogenouslyActiveNeurons_; ///< Number of endogenously active neurons.
...
///  Cereal serialization method
template <class Archive>
void Layout::serialize(Archive &archive, std::uint32_t const version) {
  archive(cereal::make_nvp("xloc", xloc_), cereal::make_nvp("yloc", yloc_),
          ...
          cereal::make_nvp("numEndogenouslyActiveNeurons",
