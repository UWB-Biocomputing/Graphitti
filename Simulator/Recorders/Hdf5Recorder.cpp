...
void Hdf5Recorder::initDataSet()
{
   ...
   dims[0] = static_cast<hsize_t>(Simulator::getInstance().totalVertices);
...
void Hdf5Recorder::compileHistories(IAllVertices &neurons)
{
   ...
   for (int iNeuron = 0; iNeuron < Simulator::getInstance().totalVertices; iNeuron++)
   {
...
void Hdf5Recorder::saveSimData(const IAllVertices &vertices)
{
    try
    {
        ...
        VectorMatrix neuronTypes(MATRIX_TYPE, MATRIX_INIT, 1, m_Simulator::getInstance().getTotalVertices(), EXC);
        for (int i = 0; i < m_Simulator::getInstance().getTotalVertices(); i++) {
            neuronTypes[i] = m_model->getLayout()->neuron_type_map[i];
        }

        // create neuron threshold matrix
        VectorMatrix neuronThresh(MATRIX_TYPE, MATRIX_INIT, 1, m_Simulator::getInstance().getTotalVertices(), 0);
        for (int i = 0; i < m_Simulator::getInstance().getTotalVertices(); i++) {
            neuronThresh[i] = dynamic_cast<const AllIFNeurons&>(vertices).Vthresh[i];
        }

        // Write the neuron location matrices
        int* iXloc = new int[m_Simulator::getInstance().getTotalVertices()];
        int* iYloc = new int[m_Simulator::getInstance().getTotalVertices()];
        for (int i = 0; i < m_Simulator::getInstance().getTotalVertices(); i++) {
        ...
        int* iNeuronTypes = new int[Simulator::getInstance().totalVertices];
        for (int i = 0; i < Simulator::getInstance().totalVertices; i++)
        {
        ...
        BGFLOAT* fNeuronThresh = new BGFLOAT[Simulator::getInstance().totalVertices];
        for (int i = 0; i < Simulator::getInstance().totalVertices; i++)
        {
