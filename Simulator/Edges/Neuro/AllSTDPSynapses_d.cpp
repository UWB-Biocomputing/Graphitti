...
///  @param  edgeIndexMapDevice    GPU address of the EdgeIndexMap on device
///  memory.
...
///  structures
///                                   on device memory.
///  @param[in] allVerticesDevice      GPU address of AllNeurons structures on
///  device memory.
///  @param[in] maxSpikes             Maximum number of spikes per neuron per
///  epoch.

__global__ void advanceSTDPSynapsesDevice(
    int totalSynapseCount, EdgeIndexMap *edgeIndexMapDevice,
...
void AllSTDPSynapses::allocEdgeDeviceStruct(void **allEdgesDevice) {
  allocEdgeDeviceStruct(allEdgesDevice,
...
///  AllSTDPSynapsesDeviceProperties struct
///                                on device memory.
...
void AllSTDPSynapses::allocEdgeDeviceStruct(void **allEdgesDevice,
  ...
  HANDLE_ERROR(
      cudaMalloc(allEdgesDevice, sizeof(AllSTDPSynapsesDeviceProperties)));
  HANDLE_ERROR(cudaMemcpy(*allEdgesDevice, &allEdgesDeviceProps,
                          sizeof(AllSTDPSynapsesDeviceProperties),
...
///  @param  numVertices            Number of vertices.
///  @param  maxEdgesPerVertex  Maximum number of synapses per neuron.
void AllSTDPSynapses::allocDeviceStruct(
    AllSTDPSynapsesDeviceProperties &allEdgesDevice, int numVertices,
    int maxEdgesPerVertex) {
  AllSpikingSynapses::allocDeviceStruct(allEdgesDevice, numVertices,
                                        maxEdgesPerVertex);

  BGSIZE maxTotalSynapses = maxEdgesPerVertex * numVertices;

  HANDLE_ERROR(cudaMalloc((void **)&allEdgesDevice.totalDelayPost_,
                          maxTotalSynapses * sizeof(int)));
  HANDLE_ERROR(cudaMalloc((void **)&allEdgesDevice.delayQueuePost_,
                          maxTotalSynapses * sizeof(BGSIZE)));
  HANDLE_ERROR(cudaMalloc((void **)&allEdgesDevice.delayIndexPost_,
                          maxTotalSynapses * sizeof(int)));
  HANDLE_ERROR(cudaMalloc((void **)&allEdgesDevice.delayQueuePost_,
                          maxTotalSynapses * sizeof(int)));
  HANDLE_ERROR(cudaMalloc((void **)&allEdgesDevice.tauspost_,
                          maxTotalSynapses * sizeof(BGFLOAT)));
  HANDLE_ERROR(cudaMalloc((void **)&allEdgesDevice.tauspre_,
                          maxTotalSynapses * sizeof(BGFLOAT)));
  HANDLE_ERROR(cudaMalloc((void **)&allEdgesDevice.taupos_,
                          maxTotalSynapses * sizeof(BGFLOAT)));
  HANDLE_ERROR(cudaMalloc((void **)&allEdgesDevice.tauneg_,
                          maxTotalSynapses * sizeof(BGFLOAT)));
  HANDLE_ERROR(cudaMalloc((void **)&allEdgesDevice.STDPgap_,
                          maxTotalSynapses * sizeof(BGFLOAT)));
  HANDLE_ERROR(cudaMalloc((void **)&allEdgesDevice.Wex_,
                          maxTotalSynapses * sizeof(BGFLOAT)));
  HANDLE_ERROR(cudaMalloc((void **)&allEdgesDevice.Aneg_,
                          maxTotalSynapses * sizeof(BGFLOAT)));
  HANDLE_ERROR(cudaMalloc((void **)&allEdgesDevice.Apos_,
                          maxTotalSynapses * sizeof(BGFLOAT)));
  HANDLE_ERROR(cudaMalloc((void **)&allEdgesDevice.mupos_,
                          maxTotalSynapses * sizeof(BGFLOAT)));
  HANDLE_ERROR(cudaMalloc((void **)&allEdgesDevice.muneg_,
                          maxTotalSynapses * sizeof(BGFLOAT)));
}
...
///  struct
///                             on device memory.
void AllSTDPSynapses::deleteDeviceStruct(
    AllSTDPSynapsesDeviceProperties &allEdgesDevice) {
...
void AllSTDPSynapses::copyEdgeHostToDevice(
    void *allEdgesDevice) { // copy everything necessary
  copyEdgeHostToDevice(allEdgesDevice,
                       Simulator::getInstance().getTotalVertices(),
                       Simulator::getInstance().getMaxEdgesPerVertex());
}
...
///  @param  maxEdgesPerVertex  Maximum number of synapses per neuron.
void AllSTDPSynapses::copyEdgeHostToDevice(
    void *allEdgesDevice, int numVertices,
    int maxEdgesPerVertex) { // copy everything necessary
  AllSTDPSynapsesDeviceProperties allEdgesDeviceProps;
  ...
  copyHostToDevice(allEdgesDevice, allEdgesDeviceProps, numVertices,
                   maxEdgesPerVertex);
}
...
///  @param  numVertices            Number of vertices.
///  @param  maxEdgesPerVertex  Maximum number of synapses per neuron.
void AllSTDPSynapses::copyHostToDevice(
    void *allEdgesDevice, AllSTDPSynapsesDeviceProperties &allEdgesDeviceProps,
    int numVertices, int maxEdgesPerVertex) { // copy everything necessary
  AllSpikingSynapses::copyHostToDevice(allEdgesDevice, allEdgesDeviceProps,
                                       numVertices, maxEdgesPerVertex);
  ...
  HANDLE_ERROR(cudaMemcpy(allEdgesDeviceProps.delayQueuePost_, delayQueuePost_,
                          maxTotalSynapses * sizeof(uint32_t),
                          cudaMemcpyHostToDevice));
...
///  @param  numVertices            Number of vertices.
///  @param  maxEdgesPerVertex  Maximum number of synapses per neuron.
void AllSTDPSynapses::copyDeviceToHost(
    AllSTDPSynapsesDeviceProperties &allEdgesDevice) {
  AllSpikingSynapses::copyDeviceToHost(allEdgesDevice);

  int numVertices = Simulator::getInstance().getTotalVertices();
  BGSIZE maxTotalSynapses =
      Simulator::getInstance().getMaxEdgesPerVertex() * numVertices;
...
///
///  @param  allEdgesDevice      GPU address of the AllEdgesDeviceProperties
///  struct
///                                 on device memory.
///  @param  allVerticesDevice       GPU address of the allNeurons struct on
///  device memory.
///  @param  edgeIndexMapDevice  GPU address of the EdgeIndexMap on device
///  memory.
void AllSTDPSynapses::advanceEdges(void *allEdgesDevice,
...
///  Set synapse class ID defined by enumClassSynapses for the caller's Synapse
///  class. The class ID will be set to classSynapses_d in device memory, and
///  the classSynapses_d will be referred to call a device function for the
///  particular synapse class.
///  Because we cannot use virtual function (Polymorphism) in device functions,
///  we use this scheme.
///  Note: we used to use a function pointer; however, it caused the growth_cuda
///  crash (see issue#137).
void AllSTDPSynapses::setEdgeClassID() {
  ...
  HANDLE_ERROR(cudaMemcpyToSymbol(classSynapses_d, &classSynapses_h,
                                  sizeof(enumClassSynapses)));
}

///  Prints GPU SynapsesProps data.
///
///  @param  allEdgesDeviceProps   GPU address of the corresponding
///  SynapsesDeviceProperties struct on device memory.
void AllSTDPSynapses::printGPUEdgesProps(void *allEdgesDeviceProps) const {
  ...
  if (size != 0) {
    ...
    HANDLE_ERROR(cudaMemcpy(sourceNeuronIndexPrint,
                            allSynapsesProps.sourceVertexIndex_,
                            size * sizeof(int), cudaMemcpyDeviceToHost));
    HANDLE_ERROR(cudaMemcpy(destNeuronIndexPrint,
                            allSynapsesProps.destVertexIndex_,
                            size * sizeof(int), cudaMemcpyDeviceToHost));
    HANDLE_ERROR(cudaMemcpy(WPrint, allSynapsesProps.W_, size * sizeof(BGFLOAT),
                            cudaMemcpyDeviceToHost));
    HANDLE_ERROR(cudaMemcpy(typePrint, allSynapsesProps.type_,
                            size * sizeof(edgeType), cudaMemcpyDeviceToHost));
    HANDLE_ERROR(cudaMemcpy(psrPrint, allSynapsesProps.psr_,
                            size * sizeof(BGFLOAT), cudaMemcpyDeviceToHost));
    HANDLE_ERROR(cudaMemcpy(inUsePrint, allSynapsesProps.inUse_,
                            size * sizeof(bool), cudaMemcpyDeviceToHost));

    HANDLE_ERROR(cudaMemcpy(decayPrint, allSynapsesProps.decay_,
                            size * sizeof(BGFLOAT), cudaMemcpyDeviceToHost));
    HANDLE_ERROR(cudaMemcpy(tauPrint, allSynapsesProps.tau_,
                            size * sizeof(BGFLOAT), cudaMemcpyDeviceToHost));
    HANDLE_ERROR(cudaMemcpy(totalDelayPrint, allSynapsesProps.totalDelay_,
                            size * sizeof(int), cudaMemcpyDeviceToHost));

    HANDLE_ERROR(cudaMemcpy(totalDelayPostPrint,
                            allSynapsesProps.totalDelayPost_,
                            size * sizeof(int), cudaMemcpyDeviceToHost));
    HANDLE_ERROR(cudaMemcpy(tauspostPrint, allSynapsesProps.tauspost_,
                            size * sizeof(BGFLOAT), cudaMemcpyDeviceToHost));
    HANDLE_ERROR(cudaMemcpy(tausprePrint, allSynapsesProps.tauspre_,
                            size * sizeof(BGFLOAT), cudaMemcpyDeviceToHost));
    HANDLE_ERROR(cudaMemcpy(tauposPrint, allSynapsesProps.taupos_,
                            size * sizeof(BGFLOAT), cudaMemcpyDeviceToHost));
    HANDLE_ERROR(cudaMemcpy(taunegPrint, allSynapsesProps.tauneg_,
                            size * sizeof(BGFLOAT), cudaMemcpyDeviceToHost));
    HANDLE_ERROR(cudaMemcpy(STDPgapPrint, allSynapsesProps.STDPgap_,
                            size * sizeof(BGFLOAT), cudaMemcpyDeviceToHost));
    HANDLE_ERROR(cudaMemcpy(WexPrint, allSynapsesProps.Wex_,
                            size * sizeof(BGFLOAT), cudaMemcpyDeviceToHost));
    HANDLE_ERROR(cudaMemcpy(AnegPrint, allSynapsesProps.Aneg_,
                            size * sizeof(BGFLOAT), cudaMemcpyDeviceToHost));
    HANDLE_ERROR(cudaMemcpy(AposPrint, allSynapsesProps.Apos_,
                            size * sizeof(BGFLOAT), cudaMemcpyDeviceToHost));
    HANDLE_ERROR(cudaMemcpy(muposPrint, allSynapsesProps.mupos_,
                            size * sizeof(BGFLOAT), cudaMemcpyDeviceToHost));
    HANDLE_ERROR(cudaMemcpy(munegPrint, allSynapsesProps.muneg_,
                            size * sizeof(BGFLOAT), cudaMemcpyDeviceToHost));
...
///  Adjust synapse weight according to the Spike-timing-dependent synaptic
///  modification induced by natural spike trains
///
///  @param  allEdgesDevice    GPU address of the
///  AllSTDPSynapsesDeviceProperties structures
///                               on device memory.
///  @param  iEdg                 Index of the synapse to set.
///  @param  delta                Pre/post synaptic spike interval.
///  @param  epost                Params for the rule given in Froemke and Dan
///  (2002).
///  @param  epre                 Params for the rule given in Froemke and Dan
///  (2002).
CUDA_CALLABLE void
stdpLearningDevice(AllSTDPSynapsesDeviceProperties *allEdgesDevice,
...
///
///  @param[in] allEdgesDevice     GPU adress of AllSTDPSynapsesDeviceProperties
///  structures
///                                   on device memory.
///  @param[in] iEdg                  Index of the Synapse to check.
///
///  @return true if there is an input spike event.
CUDA_CALLABLE bool isSTDPSynapseSpikeQueuePostDevice(
    AllSTDPSynapsesDeviceProperties *allEdgesDevice, BGSIZE iEdg) {
...
///  @param  maxSpikes              Maximum number of spikes per neuron per
///  epoch.
///
...
CUDA_CALLABLE uint64_t getSTDPSynapseSpikeHistoryDevice(
  ...
  int idxSp =
      (allVerticesDevice->spikeCount_[index] +
       allVerticesDevice->spikeCountOffset_[index] + maxSpikes + offIndex) %
      maxSpikes;
...
///
///  @param[in] totalSynapseCount        Number of synapses.
///  @param[in] edgeIndexMapDevice    GPU address of the EdgeIndexMap on device
///  memory.
///  @param[in] simulationStep           The current simulation step.
///  @param[in] deltaT                   Inner simulation step duration.
///  @param[in] allEdgesDevice        GPU address of
///  AllSTDPSynapsesDeviceProperties structures
///                                      on device memory.
///  @param[in] allVerticesDevice         GPU address of AllVertices structures
///  on device memory.
///  @param[in] maxSpikes                Maximum number of spikes per neuron per
///  epoch.
__global__ void advanceSTDPSynapsesDevice(
