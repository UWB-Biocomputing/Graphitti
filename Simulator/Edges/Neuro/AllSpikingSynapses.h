 ...
 * edge_counts.
 *
 *  For CUDA implementation, we used another structure, AllEdgesDevice, where
 * synapse parameters are stored in 1D arrays instead of 2D arrays, so that
 * device functions can access these data less latency. When copying a synapse
 * parameter, P[i][j], from host to device, it is stored in P[i *
 * max_edges_per_vertex + j] in AllEdgesDevice structure.
 *
...
typedef void (*fpPreSynapsesSpikeHit_t)(const BGSIZE,
                                        AllSpikingSynapsesDeviceProperties *);

typedef void (*fpPostSynapsesSpikeHit_t)(const BGSIZE,
                                         AllSpikingSynapsesDeviceProperties *);
...
protected:
  ...
  ///  @param  numVertices            Number of vertices.
  ///  @param  maxEdgesPerVertex  Maximum number of synapses per neuron.
  void allocDeviceStruct(AllSpikingSynapsesDeviceProperties &allEdgesDevice,
