...
void All911Edges::allocEdgeDeviceStruct(void **allEdgesDevice, int numVertices, int maxEdgesPerVertex)
{
   ...
   HANDLE_ERROR(cudaMemcpy(*allEdgesDevice, &allEdges, sizeof(All911EdgesDeviceProperties), cudaMemcpyHostToDevice));
}
...
///  @param  numVertices            Number of vertices.
///  @param  maxEdgesPerVertex  Maximum number of edges per vertex.
void All911Edges::allocDeviceStruct(All911EdgesDeviceProperties &allEdgesDevice, int numVertices, int maxEdgesPerVertex)
{
...
void All911Edges::deleteEdgeDeviceStruct(void *allEdgesDevice)
{
   ...
   HANDLE_ERROR(cudaMemcpy(&allEdgesDeviceProps, allEdgesDevice, sizeof(All911EdgesDeviceProperties), cudaMemcpyDeviceToHost));
...
///  @param  numVertices              Number of vertices.
///  @param  maxEdgesPerVertex        Maximum number of edges per vertex.
void All911Edges::copyHostToDevice(void *allEdgesDevice, All911EdgesDeviceProperties &allEdgesDeviceProps, int numVertices, int maxEdgesPerVertex)
{
   BGSIZE maxTotalEdges = maxEdgesPerVertex * numVertices;
   ...
   HANDLE_ERROR(cudaMemcpy(allEdgesDeviceProps.vertexId_, cpuVertexId, maxTotalEdges * sizeof(int), cudaMemcpyHostToDevice));

   uint64_t cpuTime[maxTotalEdges];
   for (int i = 0; i < maxTotalEdges; i++) {
      cpuTime[i] = call_[i].time;
   }
   HANDLE_ERROR(cudaMemcpy(allEdgesDeviceProps.time_, cpuTime, maxTotalEdges * sizeof(uint64_t), cudaMemcpyHostToDevice));

   int cpuDuration[maxTotalEdges];
   for (int i = 0; i < maxTotalEdges; i++) {
      cpuDuration[i] = call_[i].duration;
   }
   HANDLE_ERROR(cudaMemcpy(allEdgesDeviceProps.duration_, cpuDuration, maxTotalEdges * sizeof(int), cudaMemcpyHostToDevice));

   BGFLOAT cpuX[maxTotalEdges];
   for (int i = 0; i < maxTotalEdges; i++) {
      cpuX[i] = call_[i].x;
   }
   HANDLE_ERROR(cudaMemcpy(allEdgesDeviceProps.x_, cpuX, maxTotalEdges * sizeof(BGFLOAT), cudaMemcpyHostToDevice));

   BGFLOAT cpuY[maxTotalEdges];
   for (int i = 0; i < maxTotalEdges; i++) {
      cpuY[i] = call_[i].y;
   }
   HANDLE_ERROR(cudaMemcpy(allEdgesDeviceProps.y_, cpuY, maxTotalEdges * sizeof(BGFLOAT), cudaMemcpyHostToDevice));

   int cpuPatience[maxTotalEdges];
   for (int i = 0; i < maxTotalEdges; i++) {
      cpuPatience[i] = call_[i].patience;
   }
   HANDLE_ERROR(cudaMemcpy(allEdgesDeviceProps.patience_, cpuPatience, maxTotalEdges * sizeof(int), cudaMemcpyHostToDevice));

   int cpuOnSiteTime[maxTotalEdges];
   for (int i = 0; i < maxTotalEdges; i++) {
      cpuOnSiteTime[i] = call_[i].onSiteTime;
   }
   HANDLE_ERROR(cudaMemcpy(allEdgesDeviceProps.onSiteTime_, cpuOnSiteTime, maxTotalEdges * sizeof(int), cudaMemcpyHostToDevice));
   ...
   HANDLE_ERROR(cudaMemcpy(allEdgesDeviceProps.responderType_, cpuResponderType, maxTotalEdges * sizeof(int), cudaMemcpyHostToDevice));
   HANDLE_ERROR(cudaMemcpy(allEdgesDevice, &allEdgesDeviceProps, sizeof(All911EdgesDeviceProperties), cudaMemcpyHostToDevice));
...
void All911Edges::copyDeviceToHost(All911EdgesDeviceProperties &allEdgesDeviceProps)
{
   ...
   int cpuVertexId[maxTotalEdges];
   HANDLE_ERROR(cudaMemcpy(cpuVertexId, allEdgesDeviceProps.vertexId_, maxTotalEdges * sizeof(int), cudaMemcpyDeviceToHost));
   for (int i = 0; i < maxTotalEdges; i++) {
      call_[i].vertexId = cpuVertexId[i];
   }

   uint64_t cpuTime[maxTotalEdges];
   HANDLE_ERROR(cudaMemcpy(cpuTime, allEdgesDeviceProps.time_, maxTotalEdges * sizeof(uint64_t), cudaMemcpyDeviceToHost));
   for (int i = 0; i < maxTotalEdges; i++) {
      call_[i].time = cpuTime[i];
   }

   int cpuDuration[maxTotalEdges];
   HANDLE_ERROR(cudaMemcpy(cpuDuration, allEdgesDeviceProps.duration_, maxTotalEdges * sizeof(int), cudaMemcpyDeviceToHost));
   for (int i = 0; i < maxTotalEdges; i++) {
      call_[i].duration = cpuDuration[i];
   }

   BGFLOAT cpuX[maxTotalEdges];
   HANDLE_ERROR(cudaMemcpy(cpuX, allEdgesDeviceProps.x_, maxTotalEdges * sizeof(BGFLOAT), cudaMemcpyDeviceToHost));
   for (int i = 0; i < maxTotalEdges; i++) {
      call_[i].x = cpuX[i];
   }

   BGFLOAT cpuY[maxTotalEdges];
   HANDLE_ERROR(cudaMemcpy(cpuY, allEdgesDeviceProps.y_, maxTotalEdges * sizeof(BGFLOAT), cudaMemcpyDeviceToHost));
   for (int i = 0; i < maxTotalEdges; i++) {
      call_[i].y = cpuY[i];
   }

   int cpuPatience[maxTotalEdges];
   HANDLE_ERROR(cudaMemcpy(cpuPatience, allEdgesDeviceProps.patience_, maxTotalEdges * sizeof(int), cudaMemcpyDeviceToHost));
   for (int i = 0; i < maxTotalEdges; i++) {
      call_[i].patience = cpuPatience[i];
   }

   int cpuOnSiteTime[maxTotalEdges];
   HANDLE_ERROR(cudaMemcpy(cpuOnSiteTime, allEdgesDeviceProps.onSiteTime_, maxTotalEdges * sizeof(int), cudaMemcpyDeviceToHost));
   for (int i = 0; i < maxTotalEdges; i++) {
      call_[i].onSiteTime = cpuOnSiteTime[i];
   }

   int cpuResponderType[maxTotalEdges];
   HANDLE_ERROR(cudaMemcpy(cpuResponderType, allEdgesDeviceProps.responderType_, maxTotalEdges * sizeof(int), cudaMemcpyDeviceToHost));
   for (int i = 0; i < maxTotalEdges; i++) {
...
void All911Edges::printGPUEdgesProps(void *allEdgesDeviceProps) const
{
   ...
   if (size != 0) {
      ...
      // copy everything
      HANDLE_ERROR(cudaMemcpy(&allEdgesProps, allEdgesDeviceProps, sizeof(All911EdgesDeviceProperties), cudaMemcpyDeviceToHost));
      ...
      HANDLE_ERROR(cudaMemcpy(xPrint, allEdgesProps.x_, size * sizeof(BGFLOAT), cudaMemcpyDeviceToHost));
      HANDLE_ERROR(cudaMemcpy(yPrint, allEdgesProps.y_, size * sizeof(BGFLOAT), cudaMemcpyDeviceToHost));
